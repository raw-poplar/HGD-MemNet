...
def process_dialogue_to_tensors(dialogue_list, vocab):
    ...
    """
    if not isinstance(dialogue_list, list) or len(dialogue_list) < 2:
        return None

    # LCCC format: dialogue_list is a list of strings
    # 1. First sentence is the reference input x_ref
    x_ref_text = dialogue_list[0]
    x_ref_tensor = torch.tensor(indexesFromSentence(vocab, x_ref_text), dtype=torch.long)

    # 2. Build "thinking-output" steps
    steps_data = []
    for i in range(1, len(dialogue_list)):
        # Current input x_t is the text of the previous sentence
        x_t_text = dialogue_list[i-1]
        # Target is the text of the current sentence
        target_text = dialogue_list[i]

        # Simulate "thinking" steps for the model
...
def convert_to_binary(data_type, input_file, vocab, output_dir, num_workers=1, batch_size_multiprocessing=1000):
    """
    读取 .jsonl 文件，处理所有对话，并将它们分块保存为 .pt 文件。
    支持多进程加速。
    """
    # input_file 参数现在直接提供了完整路径
    ...
    with open(input_file, 'r', encoding='utf-8') as f:
        ...
        if num_workers > 1:
            ...
            while True:
                batch_lines = list(islice(f, batch_size_multiprocessing))
                if not batch_lines:
                    break
                # 并行处理批次
...
if __name__ == "__main__":
    try:
        ...
        parser.add_argument('--multiprocessing_batch_size', type=int, default=1000, help='Number of lines to process in a batch for multiprocessing')
        ...
        # 3. 转换所有数据集类型
        datasets_to_process = {
            "train": config.LCCC_TRAIN_FILE,
            "valid": config.LCCC_VALID_FILE,
            "test": config.LCCC_TEST_FILE
        }

        for data_type, file_path in datasets_to_process.items():
            convert_to_binary(
                data_type=data_type,
                input_file=file_path,
                vocab=vocab,
                output_dir=config.LCCC_PROCESSED_PATH,
                num_workers=args.num_workers,
                batch_size_multiprocessing=args.multiprocessing_batch_size
            )
