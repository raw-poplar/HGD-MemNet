...
def test_attention_configurations():
    ...
    for i, cfg in enumerate(attention_configs):
        ...
        if cfg['heads'] > 1 and config.DYNAMIC_GROUP_HIDDEN_DIM % cfg['heads'] != 0:
            print(f"  ⚠️  跳过: hidden_dim ({config.DYNAMIC_GROUP_HIDDEN_DIM}) 不能被头数 ({cfg['heads']}) 整除")
            print()
            continue
...
def test_attention_quality():
    ...
    for cfg in configs:
        ...
        with torch.no_grad():
            ...
            output_entropy = -torch.sum(torch.softmax(output_logits, dim=-1) * 
                                      torch.log_softmax(output_logits, dim=-1), dim=-1).mean().item()
